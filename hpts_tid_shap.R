...
other_list <- c("尿微量白蛋白", "肌酐_尿_", "尿微量白蛋白/肌酐比值")
...
predict_vars <- c(
  ...
  "白细胞计数", #"红细胞计数", 
  "血红蛋白", "血小板计数", "中性粒细胞百分比",
  #"淋巴细胞百分比", "单核细胞百分比", "嗜酸性粒细胞百分比", "嗜碱性粒细胞百分比",
  #"中性粒细胞绝对值", "淋巴细胞绝对值", #"单核细胞绝对值", "嗜酸性粒细胞绝对值",
  #"嗜碱性粒细胞绝对值", "红细胞压积", "平均红细胞体积", "平均红细胞Hb含量",
  #"平均红细胞Hb浓度", "红细胞分布宽度_CV_", 
  "红细胞分布宽度_SD_", #"血小板压积",
  #"血小板分布宽度", "平均血小板体积", "大血小板比例",
  # Liver functions
  #"直接胆红素", "间接胆红素", 
  "天门冬氨酸氨基转移酶", "丙氨酸氨基转移酶",
...
tableone_data$红细胞分布宽度_CV_ <- as.numeric(tableone_data$红细胞分布宽度_CV_)
tableone_data$红细胞分布宽度_SD_ <- as.numeric(tableone_data$红细胞分布宽度_SD_)

stop()

tableone_data <- tableone_data[, c("颈动脉靶器官损害", predict_vars)]
tableone_data <- na.omit(tableone_data)
...
model_xgboost = xgboost(
  data = as.matrix(tableone_data_train[, c(2:14)]),#训练集的自变量矩阵
...
pred_xgboost <- predict(model_xgboost, as.matrix(tableone_data_test[, c(2:14)]), type = "prob")
...
# 计算SHAP值
shap_values <- shap.values(model_xgboost, as.matrix(tableone_data_train[, c(2:14)]))

# To prepare the long-format data:
shap_long <- shap.prep(xgb_model = model_xgboost, X_train = as.matrix(tableone_data_train[, c(2:14)]),)
